...
- **実装ガイドライン**:
  ...
  - 既存コンポーネント（`Heading`, `Subheading`など）はダークモード関連のクラスを削除済み
  - 新規実装時は、`text-zinc-950`、`bg-white`、`border-zinc-200` などのライトモード専用クラスを使用

**コンポーネント使用時の注意**:
- `Heading`や`Subheading`コンポーネントは、`text-zinc-950`が自動的に適用されます
- カスタムクラスで上書きする場合は、`dark:`クラスを含めないでください
- 見出しには明示的に`text-zinc-950`を指定することを推奨（クラス競合回避のため）
