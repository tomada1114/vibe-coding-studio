...
### refactor-videos-architecture
/videos機能のアーキテクチャリファクタリング。クリーンアーキテクチャ + 軽量DDDに基づいて、現状の実装を段階的にリファクタリングします。レイヤー分離、ドメインモデル導入、リポジトリパターン適用により、メンテナンス性、拡張性、テスタビリティを大幅に向上させます。既存機能の完全互換性を保ちながら4フェーズで移行します。

- **仕様パス**: `.kiro/specs/refactor-videos-architecture/`
- **ステータス**: initialized
- **作成日**: 2025-11-05
- **注記**: 既存の動画関連機能を破壊せず、段階的に新アーキテクチャへ移行
