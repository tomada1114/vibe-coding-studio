...
| Link-button | 文字 `--link` / 下線なし、hover で下線、末尾に `↗`（12px / `--text-secondary`）| 外部リンク |

**リンク** — 本文中は `--link` + `underline`、`text-underline-offset: 0.2em`、`text-decoration-thickness: 1px`、`text-decoration-color: color-mix(in oklab, var(--link) 40%, transparent)`、hover で不透明。ナビ・一覧タイトルは既定で下線なし、hover で下線。外部リンクは末尾に `↗`（12px / `--text-secondary`。`--text-muted` はライトで 4.12:1 しかなく 13px 以下の装飾テキストにも不適のため使わない）。
