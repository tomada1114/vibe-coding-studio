...
nextjs:
  metadata:
    title: ターミナル（コマンドプロンプト）の基本操作を学ぼう
    description: ターミナルとコマンドプロンプトの基本概念と操作方法を初心者向けに解説。開き方から基本コマンド（cd、ls、pwd）まで実践的に学習します。
---
...
なお、Windowsでは`dir`というコマンドで同じような結果を得ることができますが、今後テキストでは`ls`を使用していきます。
...
なお、Windowsでは`cd`と入力するだけで同じような結果を得ることができますが、今後テキストでは`pwd`を使用していきます。
...
なお、Windowsでは`touch`コマンドが標準では利用できませんが、VS Codeのターミナルでは使用できますので、今後テキストでは`touch`を使用していきます。
