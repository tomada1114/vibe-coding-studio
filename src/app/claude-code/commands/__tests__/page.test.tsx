...
describe('Commands List Page', () => {
  describe('Rendering', () => {
    ...
    test('should render page description', () => {
      ...
      expect(
        screen.getByText(/Claude Code で使用するカスタムコマンド|custom commands for Claude Code/i)
      ).toBeInTheDocument()
    })
  ...
  describe('Navigation', () => {
    ...
    test('should link to correct detail page for each command', () => {
      ...
      const convertVideoLink = screen
        .getAllByRole('link')
        .find((link) => link.getAttribute('href')?.includes('/claude-code/commands/convert-video'))
      const prDescriptionLink = screen
        .getAllByRole('link')
        .find((link) => link.getAttribute('href')?.includes('/claude-code/commands/pr-description'))
...
describe('Commands List Page - Empty State', () => {
  ...
  test('should display empty state message when no commands exist', () => {
    ...
    expect(
      screen.getByText(/公開されているコマンドはまだありません|No commands available/i)
    ).toBeInTheDocument()
  })
