...
import { getAllCommands, getCommandBySlug, getAllCommandSlugs } from '../command-data'
...
describe('CommandLoader', () => {
  describe('getAllCommands', () => {
    ...
    it('should correctly parse frontmatter from markdown files', () => {
      ...
      if (testCommand) {
        expect(testCommand.description).toBe('Convert YouTube video data from commented format to VideoMetadata TypeScript')
        expect(testCommand.allowedTools).toEqual(['Read', 'Write', 'Edit', 'Bash'])
    ...
    it('should handle files without frontmatter gracefully', () => {
      ...
      const commandWithoutFrontmatter = commands.find(cmd => cmd.slug === 'test-command-no-frontmatter')
