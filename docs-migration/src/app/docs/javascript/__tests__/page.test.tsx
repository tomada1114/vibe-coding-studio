...
vi.mock('@/components/docs/Curriculum', () => ({
  Curriculum: ({ slug, courseContent, themeColors }: MockCurriculumProps) => (
    <div data-testid="curriculum-component">
      ...
      <div data-testid="cta-primary-link">{courseContent.cta.primaryButtonLink}</div>
      <div data-testid="cta-secondary-link">{courseContent.cta.secondaryButtonLink}</div>
      ...
      <div data-testid="features">
        {courseContent.features.map((feature, index: number) => (
          <div key={index} data-testid={`feature-${index}`}>
            ...
            <span data-testid={`feature-description-${index}`}>{feature.description}</span>
          </div>
        ))}
      </div>
      ...
      <div data-testid="target-audience">
        {courseContent.targetAudience.map((audience, index: number) => (
          <div key={index} data-testid={`audience-${index}`}>
            <span data-testid={`audience-title-${index}`}>{audience.title}</span>
            <span data-testid={`audience-description-${index}`}>{audience.description}</span>
          </div>
        ))}
      </div>
      ...
      <div data-testid="curriculum">
        {courseContent.curriculum.map((chapter, index: number) => (
          <div key={index} data-testid={`chapter-${index}`}>
            ...
            <span data-testid={`chapter-description-${index}`}>{chapter.description}</span>
          </div>
        ))}
      </div>
    </div>
...
describe('JavaScriptCurriculumPage', () => {
  ...
  describe('基本的なプロパティの確認', () => {
    ...
    it('コースタイトルと説明が正しく設定される', () => {
      render(<JavaScriptCurriculumPage />)
      expect(screen.getByTestId('course-title')).toHaveTextContent('JavaScript 基礎')
  ...
  describe('カリキュラムセクション', () => {
    it('10章すべてが正しく表示される', () => {
      ...
      expectedChapters.forEach((chapterTitle, index) => {
        expect(screen.getByTestId(`chapter-title-${index}`)).toHaveTextContent(chapterTitle)
      })
    })
    ...
    it('各章の説明にHTMLタグが含まれている', () => {
      ...
      const chapter1Description = screen.getByTestId('chapter-description-0').textContent
      expect(chapter1Description).toContain('<strong>プログラミングの基本概念</strong>')
      
      // Chapter 3の説明にstrongタグが含まれることを確認
      const chapter3Description = screen.getByTestId('chapter-description-2').textContent
      expect(chapter3Description).toContain('<strong>変数宣言（let、const）</strong>')
      
      // Chapter 7の説明にstrongタグが含まれることを確認
      const chapter7Description = screen.getByTestId('chapter-description-6').textContent
      expect(chapter7Description).toContain('<strong>DOM（Document Object Model）</strong>')
    })
  })
  ...
  describe('CTAセクション', () => {
    it('CTAリンクが正しく設定される', () => {
      ...
      // プライマリボタンのリンク（料金ページ）
      expect(screen.getByTestId('cta-primary-link')).toHaveTextContent('/pricing')
