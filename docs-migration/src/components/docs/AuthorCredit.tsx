    ...
    <div className={`mt-8 pt-4 text-xs text-slate-500 dark:border-slate-700 ${className}`}>
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Link
            href={authorUrl}
            className="font-medium text-slate-600 transition-colors hover:text-sky-600 hover:underline dark:text-slate-300 dark:hover:text-sky-400"
        ...
        {/* 日時情報（作成日・更新日） */}
        {(createdAt || updatedAt) && (
          <div className="flex flex-col gap-0.5 text-slate-400 dark:text-slate-500">
            {createdAt && (
              <div>
