...
function BasicNavLinks() {
  return (
    <div className="space-y-1">
      <Link
        href="/docs"
        className="block py-2 text-base font-semibold text-slate-700 hover:text-slate-900 dark:text-slate-100 dark:hover:text-white"
      >
        カリキュラム
      </Link>
    </div>
...
export function MobileNavigation() {
  ...
  return (
    <>
      ...
      >
        <MenuIcon className="h-5 w-5 stroke-slate-900 dark:stroke-white" />
      </button>
      <Dialog
        open={isOpen}
        onClose={setIsOpen}
