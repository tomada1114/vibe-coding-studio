...
このファイルは、このリポジトリでコードを扱う AI コーディングエージェント（Claude Code, Codex CLI 等）向けの共通ガイドです。Claude Code は `CLAUDE.md` 経由でこのファイルを読み込みます。
...
- Context7
- Claude in Chrome（標準機能 `mcp__claude-in-chrome__*`、Claude Code限定） - UIの動作確認とテストに活用可能
