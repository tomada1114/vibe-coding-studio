...
この一文が全ての判断基準になる。「方眼紙」＝構造が見えていること（罫線とセルが常に主役）。「職務経歴書」＝装飾ではなく事実（書籍・講座・動画・経歴）が読まれること。「青い光が一点だけ」＝派手さは存在するが、必ず数を制限された特権であること。
...
| **Geist Mono**（欧文）| セルの大文字モノラベル、日付・時刻、タイムスタンプ、タグ、数値メタ（動画本数・章番号・再生時間）、コードブロック、インラインコード、URL・出典ドメイン |
...
数値セル（SINCE / COURSES / VIDEOS）は Geist Mono の 40px。ここが「黒地に情報が刻まれている」印象の核になる。
...
- **詳細**: 上部に埋め込みプレイヤー（セル 1 つ分、全幅、radius 0）。以下、`VideoMetadata` の各セクションをセルに 1:1 対応させる。`learningPoints` / `timestamps` / `relatedVideos` / `udemyCourses` がそれぞれ独立したセル。**タイムスタンプは Geist Mono + tabular-nums の 2 列レイアウト**（左に `HH:MM`、右に見出し）。これが最も Mono の効く場所。

#### 講座・書籍ページ

- **書籍**: 上部に書影（左）＋ タイトル・概要・購入リンク（右）の 2 セル。以下に目次を Mono の章番号つき 1 列セル群、読者の声、著者紹介。
- **講座（クーポン）ページ**: 既存の `/coupons/*` 構造を維持しつつセル化。`[講座サムネ]` `[講座名 heading]` `[クーポンコード（Mono、`--surface-1` 地、コピーボタン）]` `[有効期限 Mono・期限が近い場合のみ `--ds-amber-700` のドット1つ]` `[CTA pill ボタン]`。クーポンコードのセルは**唯一、モノスペースが大きく（20px）出る場所**。
...
- ❌ 絵文字をセクション見出しのアイコン代わりに使う（`💡` `⏰` 等）。既存の `VideoMetadata` の絵文字つきタイトルは、表示側で Mono ラベルに置き換える。
...
| **5. プロフィール / 書籍 / 講座** | `/founder`（Blueprint Night を剥がす）→ 書籍セクション → `/coupons/*` | 旧デザインの痕跡がゼロ |
| **6. 動画** | `/videos` 一覧 → `/videos/[id]`。タイムスタンプの Mono 2 列化、絵文字見出しの Mono ラベル置換 | |
| **7. ブログ / ニュースまとめ** | 記事テンプレート（`prose` の全面上書き）→ ブログ一覧・記事 → ニュース changelog 型一覧・記事。GitHub Actions の生成物フォーマットを新テンプレートに合わせる | |
| **8. i18n** | next-intl 導入、`/en` ルーティング、hreflang、言語トグルの配線（Phase 2 で見た目だけ作っておき、ここで機能させる） | |
| **9. 後片付け** | 旧デザインの残骸削除。`docs/design/*.md`（Radiant 系）のアーカイブ化。カバレッジとアクセシビリティの最終チェック | `dark:` の使用箇所が例外リストと一致。生 hex の grep 結果がゼロ |
